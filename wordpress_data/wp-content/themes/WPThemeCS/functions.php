...
if (!function_exists('TemplateToaster_setup')) :
	 ...
	 */
        ...
        load_theme_textdomain("WPThemeCS", get_template_directory() . '/languages');
        ...
        */
        ...
        register_default_headers(array(
            'wheel' => array(
                ...
                'description' => __('Wheel', "WPThemeCS")
            ),
            'shore' => array(
                'url' => '%s/images/headers/shore.jpg',
                'thumbnail_url' => '%s/images/headers/shore-thumbnail.jpg',
                /* translators: header image description */
                'description' => __('Shore', "WPThemeCS")
            ),
            'trolley' => array(
                'url' => '%s/images/headers/trolley.jpg',
                'thumbnail_url' => '%s/images/headers/trolley-thumbnail.jpg',
                /* translators: header image description */
                'description' => __('Trolley', "WPThemeCS")
            ),
            'pine-cone' => array(
                'url' => '%s/images/headers/pine-cone.jpg',
                'thumbnail_url' => '%s/images/headers/pine-cone-thumbnail.jpg',
                /* translators: header image description */
                'description' => __('Pine Cone', "WPThemeCS")
            ),
            'chessboard' => array(
                'url' => '%s/images/headers/chessboard.jpg',
                'thumbnail_url' => '%s/images/headers/chessboard-thumbnail.jpg',
                /* translators: header image description */
                'description' => __('Chessboard', "WPThemeCS")
            ),
            'lanterns' => array(
                'url' => '%s/images/headers/lanterns.jpg',
                'thumbnail_url' => '%s/images/headers/lanterns-thumbnail.jpg',
                /* translators: header image description */
                'description' => __('Lanterns', "WPThemeCS")
            ),
            'willow' => array(
                'url' => '%s/images/headers/willow.jpg',
                'thumbnail_url' => '%s/images/headers/willow-thumbnail.jpg',
                /* translators: header image description */
                'description' => __('Willow', "WPThemeCS")
            ),
            'hanoi' => array(
                'url' => '%s/images/headers/hanoi.jpg',
                'thumbnail_url' => '%s/images/headers/hanoi-thumbnail.jpg',
                /* translators: header image description */
                'description' => __('Hanoi Plant', "WPThemeCS")
            )
        ));        
    }
...
global $post, $product;
	...
	$label = '<span class="label label-primary sale">' . __( 'Sale!', "WPThemeCS") .'</span>';
	return $label;    
	}; 
...
if (file_exists($fileName) && (isset($_POST['importt']) && isset($_POST['formdata']))) {
	...
	if( $imported ) {
		 $msg = __( "Content Imported successfully.", "WPThemeCS" ) ;
         echo $msg; 
        $imported = false; 
	}
	else {
		 $msg = __( "Content Not Imported Sucessfully, import it again", "WPThemeCS") ;
            echo $msg;
            $imported = false; 
	}
	wp_die();    
...
function TemplateToaster_continue_reading_link()
{
    if (TemplateToaster_theme_option('ttr_read_more_button')) {
        return '<br/><br/><a href="' . esc_url(get_permalink()) . '">' . '<span class="btn btn-default">' .__( TemplateToaster_theme_option('ttr_read_more'), '', "WPThemeCS") . '<span class="meta-nav">&rarr;</span></span>'. '</a>';
    } else {
        return '<br/><br/><a href="' . esc_url(get_permalink()) . '">' . __(TemplateToaster_theme_option('ttr_read_more'), '', "WPThemeCS") . '<span class="meta-nav">&rarr;</span></a>';
...
<?php echo(__('Navigation', "WPThemeCS")); ?>
</h3>
<?php endif; ?>
<?php
            if ( ($nav_id == 'nav-above' && TemplateToaster_theme_option('ttr_pagination_link_posts_above')) || ($nav_id == 'nav-below' && TemplateToaster_theme_option('ttr_pagination_link_posts_below')) ){
                ...
                $pge = paginate_links(array(
                    ...
                    'prev_next' => True,
                    'format' => '?paged=%#%',
                    'prev_text' => __('Previous', "WPThemeCS"),
                    'next_text' => __('Next', "WPThemeCS"),
...
<?php next_posts_link(__('<span class="meta-nav">&larr;</span> Older posts', "WPThemeCS")); ?>
</div>
  <div
      class="nav-next">
<?php previous_posts_link(__('Newer posts <span class="meta-nav">&rarr;</span>', "WPThemeCS")); ?>
</div>
<?php } ?>
</nav>
<!-- #nav-above -->
...
<li class="post pingback">
  <p>
<?php _e('Pingback:', "WPThemeCS"); ?>
<?php comment_author_link(); ?>
<?php edit_comment_link(__('Edit', "WPThemeCS"), '<span class="edit-link">', '</span>'); ?>
</p>
<?php
    break;
    default :
...
<?php
        ...
        /* translators: 1: comment author, 2: date and time */
                            printf(__('%1$s on %2$s <span class="says">said:</span>', "WPThemeCS"),
                                sprintf('<span class="fn">%s</span>', get_comment_author_link()),
                                sprintf('<a href="%1$s"><time datetime="%2$s">%3$s</time></a>',
                                    esc_url(get_comment_link($comment->comment_ID)),
                                    get_comment_time('c'),
                /* translators: 1: date, 2: time */
                                    sprintf(__('%1$s at %2$s', "WPThemeCS"), get_comment_date(), get_comment_time())
            )
        );
        ?>
<?php edit_comment_link(__('Edit', "WPThemeCS"), '<span class="edit-link">', '</span>'); ?>
...
<?php _e('Your comment is awaiting moderation.', "WPThemeCS"); ?>
</em>
<br/>
<?php endif; ?>
<!--
...
<?php comment_text(); ?>
    ...
    <div class="reply">
<?php comment_reply_link(array_merge($args, array('reply_text' => __('Reply <span>&darr;</span>', "WPThemeCS"), 'depth' => $depth, 'max_depth' => $args['max_depth']))); ?>
    </div>
    <!-- .reply -->
  </div>
  <!-- #comment-## -->
...
if (!function_exists('TemplateToaster_entry_meta')) :
    ...
    function TemplateToaster_entry_meta()
    {
        if (is_sticky() && is_home() && !is_paged())
            echo '<span class="featured-post">' . __('Sticky', "WPThemeCS") . '</span>';
        ...
        if (!has_post_format(array('chat', 'status'))):
            $categories_list = get_the_category_list(__(', ', "WPThemeCS"));
            if ($categories_list) {
        ...
        // Translators: used between list items, there is a space after the comma.
        $tag_list = get_the_tag_list('', __(', ', "WPThemeCS"));
        if ($tag_list) {
        ...
        if (!has_post_format(array('chat', 'status', 'aside', 'quote'))):
            if ('post' == get_post_type()) {
                printf('<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author"> %3$s | </a></span>',
                    ...
                    esc_attr(sprintf(__('View all posts by %s', "WPThemeCS"), get_the_author())),
                    get_the_author()
                );
            }
        endif;
...
if (!function_exists('TemplateToaster_entry_date')) :
    ...
    {
        if (has_post_format(array('chat', 'status')))
            $format_prefix = _x('%1$s on %2$s ', '1: post format name. 2: date', "WPThemeCS");
        else
            $format_prefix = '%2$s ';
        ...
        if (TemplateToaster_theme_option('ttr_remove_date')):
            $date = sprintf('<span class="date"><a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s">%4$s</time></a></span>',
                ...
                esc_attr(sprintf(__('Permalink to %s', "WPThemeCS"), the_title_attribute('echo=0'))),
    ...
    function TemplateToaster_posted_on($date, $author)
    {
        ...
        $post_url =  urldecode(esc_url( get_permalink() ) );
            ...
            if (is_sticky() && is_home() && !is_paged()) {
                ...
                echo '<span style="clear:both;">' . __('Sticky', "WPThemeCS") . '</span>';
            }
            if ( get_the_time() !== get_the_modified_time() ) 
			{
				if ($date && $author)
				{
					...
					{
						$time_string = '<a href="' . $post_url . '" rel="bookmark"><img alt="' . __('date', "WPThemeCS") . '" src="' . get_template_directory_uri() . '/images/datebutton.png"/><time class="updated" datetime="%3$s">%4$s</time><time class="entry-date post_updated published hidden d-none" datetime="%1$s">%2$s</time></a><span class = "meta">  ' . __('by ', "WPThemeCS") . ' </span><span class="author vcard">  <img alt="' . __('author ', "WPThemeCS") . '" src="' . get_template_directory_uri() . '/images/authorbutton.png"/>  <a href="%5$s" title="%6$s" class="url fn n" rel="author">%7$s</a></span>';
					   $post_status= __('Updated on ', "WPThemeCS");
					}
					else if ($var_author)
					{
					  $time_string = '<span class = "meta">  ' . __('by ', "WPThemeCS") . ' </span><span class="author vcard">  <img alt="' . __('author ', "WPThemeCS") . '" src="' . get_template_directory_uri() . '/images/authorbutton.png"/>  <a href="%5$s" title="%6$s" class="url fn n" rel="author">%7$s</a></span>';
					}
					else if ($var_date)
					{
					   $time_string = '<img alt="' . __('date', "WPThemeCS") . '" src="' . get_template_directory_uri() . '/images/datebutton.png"/><time class="updated" datetime="%3$s">%4$s</time><time class="entry-date post_updated published hidden d-none" datetime="%1$s">%2$s</time>';
					  $post_status= __('Updated on ', "WPThemeCS");
					}
				}
				else if ($date && !$author) 
				{
					if ($var_date && $var_author)
					{
						$time_string = '<a href="' . $post_url . '" rel="bookmark"><img alt="' . __('date', "WPThemeCS") . '" src="' . get_template_directory_uri() . '/images/datebutton.png"/><time class="updated" datetime="%3$s">%4$s</time><time class="entry-date post_updated published hidden d-none" datetime="%1$s">%2$s</time></a><span class = "meta">  ' . __('by ', "WPThemeCS") . ' </span><span class="author vcard"> <a href="%5$s" title="%6$s" class="url fn n" rel="author">%7$s</a></span>';
					   $post_status= __('Updated on ', "WPThemeCS");
					}
					else if ($var_author)
					{
					  $time_string = '<span class = "meta">  ' . __('by ', "WPThemeCS") . ' </span><span class="author vcard"><a href="%5$s" title="%6$s" class="url fn n" rel="author">%7$s</a></span>';
					}
					else if ($var_date)
					{
					   $time_string = '<img alt="' . __('date', "WPThemeCS") . '" src="' . get_template_directory_uri() . '/images/datebutton.png"/><time class="updated" datetime="%3$s">%4$s</time><time class="entry-date post_updated published hidden d-none" datetime="%1$s">%2$s</time>';
				...
				else
				{
					if ($var_date && $var_author)
					{
						$time_string = '<a href="' . $post_url . '" rel="bookmark"><time class="updated" datetime="%3$s">%4$s</time><time class="entry-date post_updated published hidden d-none" datetime="%1$s">%2$s</time></a><span class = "meta">  ' . __('by ', "WPThemeCS") . ' </span><span class="author vcard"><a href="%5$s" title="%6$s" class="url fn n" rel="author">%7$s</a></span>';
					   $post_status= __('Updated on ', "WPThemeCS");
					}
					else if ($var_author)
					{
			...
			else
			{
				if ($date && $author)
				{
					if ($var_date && $var_author)
					{
						$time_string='<a href="' . $post_url . '" rel="bookmark"><img alt="' . __('date', "WPThemeCS") . '" src="' . get_template_directory_uri() . '/images/datebutton.png"/><time class="entry-date published" datetime="%1$s">%2$s</time><time class="post_published updated hidden d-none" datetime="%3$s">%4$s</time></a><span class = "meta">  ' . __('by ', "WPThemeCS") . ' </span> <span class="author vcard"><img alt="' . __('author ', "WPThemeCS") . '" src="' . get_template_directory_uri() . '/images/authorbutton.png"/>   <a href="%5$s" title="%6$s" class="url fn n" rel="author">%7$s</a></span>';
					$post_status=  __('Posted on ', "WPThemeCS");
					}
					else if ($var_author)
					{
					  $time_string = '<span class = "meta">  ' . __('by ', "WPThemeCS") . ' </span><span class="author vcard">  <img alt="' . __('author ', "WPThemeCS") . '" src="' . get_template_directory_uri() . '/images/authorbutton.png"/>  <a href="%5$s" title="%6$s" class="url fn n" rel="author">%7$s</a></span>';
					...
					{
					   $time_string='<img alt="' . __('date', "WPThemeCS") . '" src="' . get_template_directory_uri() . '/images/datebutton.png"/><time class="entry-date published" datetime="%1$s">%2$s</time><time class="post_published updated hidden d-none" datetime="%3$s">%4$s</time>';
					$post_status=  __('Posted on ', "WPThemeCS");
					}
				}
				else if ($date && !$author)
				{
					if ($var_date && $var_author)
					{
						$time_string='<a href="' . $post_url . '" rel="bookmark"><img alt="' . __('date', "WPThemeCS") . '" src="' . get_template_directory_uri() . '/images/datebutton.png"/><time class="entry-date published" datetime="%1$s">%2$s</time><time class="post_published updated hidden d-none" datetime="%3$s">%4$s</time></a><span class = "meta">  ' . __('by ', "WPThemeCS") . ' </span> <span class="author vcard"><a href="%5$s" title="%6$s" class="url fn n" rel="author">%7$s</a></span>';
					$post_status=  __('Posted on ', "WPThemeCS");
					}
					else if ($var_author)
					{
				...
				elseif (!$date && $author)
				{
					if ($var_date && $var_author)
					{
						$time_string='<a href="' . $post_url . '" rel="bookmark"><time class="entry-date published" datetime="%1$s">%2$s</time><time class="post_published updated hidden d-none" datetime="%3$s">%4$s</time></a><span class = "meta">  ' . __('by ', "WPThemeCS") . ' </span> <span class="author vcard"><img alt="' . __('author ', "WPThemeCS") . '" src="' . get_template_directory_uri() . '/images/authorbutton.png"/>   <a href="%5$s" title="%6$s" class="url fn n" rel="author">%7$s</a></span>';
					$post_status=  __('Posted on ', "WPThemeCS");
					}
					else if ($var_author)
					{
				...
				else
				{
					if ($var_date && $var_author)
					{
						$time_string='<a href="' . $post_url . '" rel="bookmark"><time class="entry-date published" datetime="%1$s">%2$s</time><time class="post_published updated hidden d-none" datetime="%3$s">%4$s</time></a><span class = "meta">  ' . __('by ', "WPThemeCS") . ' </span> <span class="author vcard"><a href="%5$s" title="%6$s" class="url fn n" rel="author">%7$s</a></span>';
					$post_status=  __('Posted on ', "WPThemeCS");
					}
					else if ($var_author)
					{
			...
			$time_string = sprintf( $time_string,
					...
					esc_url(get_author_posts_url(get_the_author_meta('ID'))),
					sprintf(esc_attr__('View all posts by %s', "WPThemeCS"), esc_html(get_the_author())),
					esc_html(get_the_author())
					);
					
					
			// Wrap the time string in a link, and preface it with 'Posted on' or 'Updated On'.
			printf(__( '<span class="meta">%s</span>%s', 'WPThemeCS' ),$post_status,$time_string );
...
/* function TemplateToaster_generate_menu( $meenu, $TemplateToaster_magmenu, $TemplateToaster_menuh, $TemplateToaster_vmenuh, $TemplateToaster_ocmenu, $TemplateToaster_cssprefix = "ttr_")
{
    ...
    $output = '';
    if (is_front_page()) {
        $output .= '<li class="' . $TemplateToaster_cssprefix . $meenu . '_items_parent dropdown active"><a href="' . get_home_url(null, '/') . '" class="' . $TemplateToaster_cssprefix . $meenu . '_items_parent_link_active"><span class="menuchildicon"></span>' . __('Home', "WPThemeCS");
	  ...
	  //  $output .= ('</a><hr class="horiz_separator" />'); 
        ...
        $output .= '<li class="' . $TemplateToaster_cssprefix . $meenu . '_items_parent dropdown"><a href="' . get_home_url(null, '/') . '" class="' . $TemplateToaster_cssprefix . $meenu . '_items_parent_link"><span class="menuchildicon"></span>' . __('Home', "WPThemeCS");
...
function TemplateToaster_theme_comment_form($args = array(), $post_id = null)
{
    ...
    $fields = array(
        'author' => '<p class="comment-form-author">' . '<label for="author">' . __('Name', "WPThemeCS") . '</label> ' . ($req ? '<span class="required">*</span>' : '') . '<br/>' .
            '<input id="author" name="author" type="text" value="' . esc_attr($commenter['comment_author']) . '" size="30"' . $aria_req . ' /></p>',
        'email' => '<p class="comment-form-email"><label for="email">' . __('Email', "WPThemeCS") . '</label> ' . ($req ? '<span class="required">*</span>' : '') . '<br/>' .
            '<input id="email" name="email" type="text" value="' . esc_attr($commenter['comment_author_email']) . '" size="30"' . $aria_req . ' /></p>',
        'url' => '<p class="comment-form-url"><label for="url">' . __('Website', "WPThemeCS") . '</label>' . '<br/>' .
            '<input id="url" name="url" type="text" value="' . esc_attr($commenter['comment_author_url']) . '" size="30" /></p>',
    );

    $required_text = sprintf(' ' . __('Required fields are marked %s', "WPThemeCS"), '<span class="required">*</span>');
    $defaults = array(
        'fields' => apply_filters('comment_form_default_fields', $fields),
        'comment_field' => '<p class="comment-form-comment"><label for="comment">' . _x('Comment', 'noun', "WPThemeCS") . '</label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>' . '<br/>',
        'must_log_in' => '<p class="must-log-in">' . sprintf(__('You must be <a href="%s">logged in</a> to post a comment.', "WPThemeCS"), wp_login_url(apply_filters('the_permalink', get_permalink($post_id)))) . '</p>',
        'logged_in_as' => '<p class="logged-in-as">' . sprintf(__('Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', "WPThemeCS"), admin_url('profile.php'), $user_identity, wp_logout_url(apply_filters('the_permalink', get_permalink($post_id)))) . '</p>',
        'comment_notes_before' => '<p class="comment-notes">' . __('Your email address will not be published.', "WPThemeCS") . ($req ? $required_text : '') . '</p>',
        'comment_notes_after' => '<p class="form-allowed-tags">' . sprintf(__('You may use these <abbr title="HyperText Markup Language">HTML</abbr> tags and attributes: %s', "WPThemeCS"), ' <code>' . allowed_tags() . '</code>') . '</p>',
        'id_form' => 'commentform',
        'id_submit' => 'submit',
        'title_reply' => __('Leave a Reply', "WPThemeCS"),
        'title_reply_to' => __('Leave a Reply to %s', "WPThemeCS"),
        'cancel_reply_link' => __('Cancel reply', "WPThemeCS"),
        'label_submit' => __('Post Comment', "WPThemeCS"),
    );
...
function TemplateToaster_add_init()
{
    ...
    if ($screen->id == 'appearance_page_mytheme-options') {
        ...
        $tt_upload_data = array('title' => __('Choose or Upload an Image', "WPThemeCS"), 'text' => __('Use this image', "WPThemeCS"));
...
function TemplateToaster_replace_thickbox_text($translated_text, $text, $domain)
{
    if ('Insert into Post' == $text) {
        ...
        if ($referer != '') {
            return __('Select this image!', "WPThemeCS");
        }
    }
...
function TemplateToaster_customAdmin()
{
    ...
    $passed_data = array('on' => __('ON', "WPThemeCS"), 'off' => __('OFF', "WPThemeCS"), 'pageClass' => $pageClass);
        }
        else
	  $passed_data = array('on' => __('ON', "WPThemeCS"), 'off' => __('OFF', "WPThemeCS"));
   ...
   // wp_enqueue_script('jquery_tinymce_script');
	...
	if ($islisticon) {
		echo '{list-style-image:url('. get_stylesheet_directory_uri() . '/images/43434194listimg.png);}';
	} else {
...
function TemplateToaster_wordpress_breadcrumbs()
    ...
    $name = __('Home', "WPThemeCS"); //text for the 'Home' link
    ...
    if (!is_home() && !is_front_page() || is_paged()) {
        ...
        if (is_category()) {
            ...
            echo $currentBefore . __('Archive by category &#39;', "WPThemeCS");
            single_cat_title();
            echo '&#39;' . $currentAfter;
            ...
            echo $currentBefore . __('Search results for &#39;', "WPThemeCS") . get_search_query() . '&#39;' . $currentAfter;

        } elseif (is_tag()) {
            echo $currentBefore . __('Posts tagged &#39;', "WPThemeCS");
            single_tag_title();
            echo '&#39;' . $currentAfter;
            ...
            echo $currentBefore . __('Articles posted by', "WPThemeCS") . $userdata->display_name . $currentAfter;

        } elseif (is_404()) {
            echo $currentBefore . __('Error 404', "WPThemeCS") . $currentAfter;
		}

        if (get_query_var('paged')) {
            if (is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author()) echo ' (';
            echo __('Page', "WPThemeCS") . ' ' . get_query_var('paged');
            if (is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author()) echo ')';
        }
...
function TemplateToaster_in_widget_form($t, $return, $instance)
{
    ...
    $wp_page_types = array(
        'front' => __('Front', "WPThemeCS"),
        'home' => __('Blog', "WPThemeCS"),
        'archive' => __('Archives', "WPThemeCS"),
        'single' => __('Single Post', "WPThemeCS"),
        '404' => __('404', "WPThemeCS")
    );

    ?>
<br/>
<label>
<?php echo(__('Hide widget on:', "WPThemeCS")); ?>
</label>
...
<div class="menupageelement">
        ...
        <label class="widgetlabel"
          ...
          "><?php echo $label . ' ' . __('Page', "WPThemeCS") ?>
</label>
        </div>
<?php } ?>
</div>
...
<?php if (!isset($instance['style']))
        $instance['style'] = null;
	?>
<label for="<?php echo $t->get_field_id('style'); ?>"><?php echo(__('Block Style:', "WPThemeCS")); ?>
</label>
  <select id="<?php echo $t->get_field_id('style'); ?>" name="<?php echo $t->get_field_name('style'); ?>">
    <option
      <?php selected($instance['style'], 'default'); ?>value="default"><?php echo(__('Default', "WPThemeCS")); ?>
</option>
    <option
      <?php selected($instance['style'], 'none'); ?>
      value="none"><?php echo(__('None', "WPThemeCS")); ?>
</option>
    <option
      <?php selected($instance['style'], 'block'); ?>value="block"><?php echo(__('Block', "WPThemeCS")); ?>
</option>
        </select>
<?php
    $retrun = null;
...
function TemplateToaster_wp_title($title, $sep)
{
    ...
    if (($paged >= 2 || $page >= 2) && !is_404()) {
        $title .= " $sep " . sprintf(__('Page %s', "WPThemeCS"), max($paged, $page));
    }
...
<span class="url"><a class="<?php echo $TemplateToaster_cssprefix; ?>author_name" href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ) ?>"></a><?php
    echo get_comment_author_link( $comment->comment_ID ); ?>
                ...
                <time datetime="<?php echo get_comment_date("c")?>" class="comment-date">
                    <a class="<?php echo $TemplateToaster_cssprefix; ?>comment_date" href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ) ?>"><?php printf(__('%1$s at %2$s', "WPThemeCS"), get_comment_date(),  get_comment_time()) ?></a> </time>
                <hr style="color:#fff;"/>
<?php comment_text() ?>
<div style="clear:both"></div>
                <hr style="color:#fff;">
                ...
                <div class="<?php echo $TemplateToaster_cssprefix; ?>comment_reply_edit">
                    ...
                    <span class="<?php echo $TemplateToaster_cssprefix; ?>reply_edit"><?php edit_comment_link(__('(Edit)', "WPThemeCS")) ?></span>
                </div>
<?php if ($comment->comment_approved == '0') : ?>
<br /><em><?php _e('Your comment is awaiting approval.', "WPThemeCS") ?></em>
<?php endif; ?>
</div>
...
function theme_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'colorscheme', array(
		'type'    => 'radio',
		'label'    => __( 'Color Scheme', 'WPThemeCS' ),
		'choices'  => array(
			'light'  => __( 'Light', 'WPThemeCS' ),
			'custom' => __( 'Custom', 'WPThemeCS' ),
		),
		'section'  => 'colors',
		'priority' => 5,
	) );
...
function templatetoaster_import_xml()
{
	...
	if ($_POST['data'] != NULL)
	{
		...
		$load_theme_options=simplexml_load_string($LoadXML1) or die(__('Error: Cannot create object','WPThemeCS'));
		...
		if ($load_theme_options->option)
		{
			...
			if($result)
			{
				die(__('Theme options imported successfully','WPThemeCS'));				
			}
			else
			{
				die(__('Problem in importing theme options.','WPThemeCS'));	
			}
		}
		else
		{
			die(__('Incorrect XML file.','WPThemeCS'));
		}	
	}
	else
	{
		die(__('Please, upload an XML file only','WPThemeCS'));
	}
}
