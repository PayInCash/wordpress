...
//Theme_Content_Import
	...
	{	
		...
		$post_mail = array(
			'subject' => sprintf(
				_x( '%1$s "%2$s"', 'mail subject', 'WPThemeCS' ),
				get_bloginfo( 'name' ), '['.$mail_arr[3].']' ),
			'sender' => sprintf( '['.$mail_arr[1].'] <%s>', '['.$mail_arr[2].']' ),
			'body' =>
				sprintf( __( 'From: %s', 'WPThemeCS' ),
					'['.$mail_arr[1].'] <['.$mail_arr[2].']>' ) . "\n"
				. sprintf( __( 'Subject: %s', 'WPThemeCS' ),
					'['.$mail_arr[3].']' ) . "\n\n"
				. __( 'Message Body:', 'WPThemeCS' )
					. "\n" . '[your-message]' . "\n\n"
				. '-- ' . "\n"
				. sprintf( __( 'This e-mail was sent from a contact form on %1$s (%2$s)',
					'WPThemeCS' ), get_bloginfo( 'name' ), home_url() ),
